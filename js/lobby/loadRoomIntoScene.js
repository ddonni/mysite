...
// 각 가구를 채움 — 이젤 보드, 턴테이블 LP, 냉장고 문, 벽 액자 3개,
// 책장. sceneSetters는 buildScene()이 돌려준 set* 함수들을 그대로
// 넘기면 됨. 넷 다 독립적인 요청이라 하나가 실패해도(catch) 나머지는
// 그대로 채워짐 — 로비 자체가 어느 하나의 실패로 멎지 않게 함.
export function loadRoomIntoScene(ctx, sceneSetters) {
  const { setSketchbookPreview, setFeaturedSong, setFeaturedFood, setFeaturedWorks, setLibraryBooks } = sceneSetters;
  ...
  // 냉장고 문에 인생 음식(있으면)/가장 최근 음식 사진을 채워넣음 —
  // 음악처럼 자기만의 가구(냉장고)가 있어서 액자/책장 후보에서는 뺌.
  fetch(`${roomApi(ctx)}/records?cat=food`)
    .then((res) => (res.ok ? res.json() : []))
    .then((records) => { if (records && records.length) setFeaturedFood(records.find((r) => r.featured) || records[0]); })
    .catch(() => {}); // 실패해도 냉장고는 기본 상태로 남을 뿐, 로비 자체는 멀쩡히 작동함
  ...
  //  - 책장: 음악·음식을 뺀 나머지 기록 전부의 제목으로 그 개수만큼만 채움.
  fetch(`${roomApi(ctx)}/records`)
    .then((res) => (res.ok ? res.json() : []))
    .then((records) => {
      const list = records || [];
      const works = list.filter((r) => r.cat !== 'music' && r.cat !== 'food');
      // "인생 음식"으로 지정된 food 기록은 음악과 마찬가지로 액자 후보가
      // 아니라 자기 가구(냉장고)에 걸리는 거라서, works 안에서만 골라야
      // 함(list 전체에서 고르면 인생 음식이 잘못 액자에 걸릴 수 있음).
